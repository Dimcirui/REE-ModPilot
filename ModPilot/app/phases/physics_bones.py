...
class PhysicsChains(PhaseTool):
    ...
    def run(
        ...
        # ── prepare_only branch: cleanup → verify → auto-retry once ──────────
        if prepare_only:
            state_before = cache.refresh()
            try:
                ...
                # Attempt 1: cleanup + refresh
                err = self._clear_and_refresh_chain_roles(client, target_arm)
                if err is not None:
                    return PhaseResult.fail(err)

                # Verify: all chain_role='head'/'branch_head' bones must have
                # an _End bone descendant (auto-generated by Phase 3.5). Body
                # bones that were accidentally marked have no _End descendants.
                verify_err, clean = self._verify_chain_marks(client, target_arm)
                if verify_err is not None:
                    # Non-fatal: treat as unknown state, surface warning
                    clean = False

                if not clean:
                    # Attempt 2: retry cleanup + refresh, then re-verify
                    err = self._clear_and_refresh_chain_roles(client, target_arm)
                    if err is not None:
                        return PhaseResult.fail(err)
                    verify_err, clean = self._verify_chain_marks(client, target_arm)
            ...
            state_after = cache.refresh()
            diff = state_before.diff(state_after)
            diff["marks_clean"] = clean
            if clean:
                diff["message"] = (
                    "Chain role marks verified clean: all chain heads have _End "
                    "bone descendants; no body bones are incorrectly marked. "
                    "Proceed with physics_chains (with inferred_types) to create "
                    "RE Chain structures."
                )
            else:
                diff["message"] = (
                    "Warning: chain role marks could not be fully verified after "
                    "two cleanup attempts. Some bones may still be incorrectly "
                    "marked (body bones without _End descendants detected). "
                    "Please inspect bone colors in Blender and remove any "
                    "incorrect chain_role marks manually before proceeding."
                )
            return PhaseResult.ok(diff)
