...
class MaterialSetup(PhaseTool):
    ...
    def tool_schema(cls) -> dict[str, Any]:
        return {
            "name": "material_setup",
            "description": (
                "Phase 5B: Wire textures into Principled BSDF nodes for each material. "
                "Accepts a confirmed texture mapping from the agent loop. "
                "Normal slot wiring inserts a Y-inversion chain for VRChat (OpenGL normals). "
                "MMD models skip this tool."
            ),
            "input_schema": {
                "type": "object",
                "properties": {
...
class MaterialGenerate(PhaseTool):
    ...
    def tool_schema(cls) -> dict[str, Any]:
        return {
            ...
            "description": (
                "Phase 5C: Run the MDF2 generator on a mesh collection. "
                "Calls mdf_gen_refresh() to auto-detect node strategies, applies the "
                "confirmed preset_mapping, then calls mdf_gen_process(). "
                "Returns the MDF2 collection name for use in batch_export."
            ),
            "input_schema": {
                "type": "object",
                "properties": {
                    "mesh_collection": {
                        "type": "string",
                        "description": "Blender collection name containing the mesh objects.",
                    },
                    "texture_base_path": {
                        "type": "string",
                        "description": (
                    ...
                    "natives_root": {
                        "type": "string",
                        "description": (
                            "Absolute path to the directory containing the 'natives' folder. "
                            "If omitted, reads scene['mhws_natives_root'] (set by batch_export)."
                        ),
                    },
                },
                "required": ["mesh_collection", "texture_base_path", "preset_mapping"],
            },
        }
    ...
    def _run_generator(
        ...
        *,
        ...
        body = textwrap.dedent("""\
            ...
            if col is None:
                print(_sentinel)
                print("PRECONDITION:collection_not_found:" + _col_name)
            else:
                ...
                # Resolve preset paths via RE Mesh Editor's load_preset_enum_items
                _load_fn = None
                for _mod in sys.modules.values():
                    if hasattr(_mod, "load_preset_enum_items"):
                        _load_fn = _mod.load_preset_enum_items
                        break
                preset_lookup = {}
                ...
                # Apply confirmed preset mapping
                auto_guessed = []
                for entry in s.material_list:
                    mat_name = entry.material.name if entry.material else ""
                ...
                try:
                    ...
                    processed = [e.material.name for e in s.material_list if e.material]
                    mdf_col = s.mdf_collection_name
                    print(_sentinel)
                    if "FINISHED" not in str(ret):
                        print("CANCELLED:" + str(ret))
                    else:
        ...
        if lines[0].startswith("CANCELLED:"):
            return (
                PhaseError(
                    category="operator_failed",
                    operator=_OP_MDF_PROCESS,
                    message=f"MDF2 generator did not finish: {lines[0]}",
                ),
                {},
            )
        if lines[0].startswith("RESULT:"):
            try:
                data = json.loads(lines[0][len("RESULT:"):])
                return None, data
            except json.JSONDecodeError:
                return (
                    PhaseError(
        ...
        return (
            PhaseError(
                category="unexpected",
                operator=_OP_MDF_PROCESS,
                message=f"Unexpected output from MDF2 generator: {lines[0]!r}",
            ),
            {},
        )
