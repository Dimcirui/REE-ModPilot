...
- ⚪ **Material widget empty rows** — Phase 5 widget renders empty `<select>` cells because there is no `_suggest_texture_mapping` equivalent (unlike Phase 4A's `_annotate_chains`). Needs LLM-assisted pre-fill pass before emitting `widget_material` SSE event. (~80 LOC + template update)
